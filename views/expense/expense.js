...
window.addEventListener("DOMContentLoaded", () => {
  ...
  axios
    .get(
      `https://13.233.61.14:3306/user/expense/getExpense?page=${page}&perPage=${expensePerPage}`,
      config
    )
    .then((res) => {
...
function premiumFunction() {
  leaderBoardBtn.addEventListener("click", () => {
    if (leaderBoardBtn.innerText === "Show LeaderBoard") {
      ...
      axios
        .get("https://13.233.61.14:3306/premium/leaderBoardSum", config)
        .then((res) => {
  ...
  fileHistoryBtn.addEventListener("click", () => {
    ...
    if (fileHistoryBtn.innerText == "Show File History") {
      ...
      axios
        .get("https://13.233.61.14:3306/user/fileHistory", config)
        .then((result) => {
...
function saveExpense(e) {
  ...
  axios
    .post(
      "https://13.233.61.14:3306/user/expense/addExpense",
      expenseDetails,
      config
    )
    .then((res) => {
...
function toCreateListItem(expenseData) {
  ...
  deleteButton.addEventListener("click", function () {
    axios
      .delete(
        `https://13.233.61.14:3306/user/expense/deleteExpense/${expenseData.id}`,
        config
      )
      .then((res) => {
...
document.getElementById("buy_premium").onclick = async function (e) {
  ...
  const response = await axios.get(
    "https://13.233.61.14:3306/user/purchase/premiumMember",
    config
  );
  ...
  var options = {
    ...
    handler: async function (response) {
      await axios.post(
        "https://13.233.61.14:3306/user/purchase/updateTranscationStatus",
        {
...
function downloadFile() {
  axios
    .get("https://13.233.61.14:3306/user/downloadFile", config)
    .then((res) => {
      if (res.status === 201) {
        var a = document.createElement("a");
